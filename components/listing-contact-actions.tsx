...
function parseContact(contactMethod: string) {
  const emailMatch = contactMethod.match(/(?:邮箱|Email)\s*([^/]+)/i);
  const wechatMatch = contactMethod.match(/(?:微信|WeChat)\s*([^/]+)/i);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-6 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[2rem] bg-white p-6 shadow-2xl shadow-slate-950/20">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm uppercase tracking-[0.25em] text-emerald-700">Contact</p>
            ...
            <div className="mt-6 space-y-4 text-sm text-slate-600">
              ...
              <div className="rounded-2xl bg-slate-50 p-4">
                <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Phone / WeChat</p>
