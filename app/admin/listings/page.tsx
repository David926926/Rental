    ...
    return (
      <div className="mx-auto max-w-3xl px-6 py-16">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm">
          <h1 className="text-3xl font-semibold text-slate-900">Admin access only</h1>
          <p className="mt-3 leading-7 text-slate-600">Please log in with an authorized admin account to view the review queue.</p>
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12">
      <div className="mb-8">
        ...
        <h1 className="mt-3 text-4xl font-semibold text-slate-900">Listing Review Dashboard</h1>
        <p className="mt-3 text-slate-600">Review pending listings, update statuses, and handle reports.</p>
      </div>
      ...
      <section className="mb-8 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 space-y-3">
          {reports.length > 0 ? (
            reports.map((report) => {
              ...
              return (
                <div key={report.id} className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
                  <p className="font-medium text-slate-900">{listing?.title ?? "Unknown listing"}</p>
            ...
            <p className="text-sm text-slate-500">There are no reports right now.</p>
          )}
        </div>
      </section>
              ...
              <div>
                ...
                <div className="mt-5 grid gap-3 md:grid-cols-4">
                  ...
                  <div className="rounded-2xl bg-slate-50 p-4">
                    ...
                    <p className="mt-1 font-medium text-slate-900">{publisher?.name ?? "Unknown"}</p>
                  </div>
                  <div className="rounded-2xl bg-slate-50 p-4">
